...
import pyperclip
import time
from PIL import ImageGrab, Image, ImageChops, ImageStat, ImageFile
...
# === base64エンコードされた↑アイコン画像（32x38 px） ===
arrow_reference_base64 = (
    "iVBORw0KGgoAAAANSUhEUgAAACAAAAAmCAYAAAClI5npAAACHUlEQVR4nO2YXVPaQBSG390NCQE6WoMdJHwUHZn+"
    "/7/BtTN01NiWb2vFAoFkl2R70XYkJprFaeNNzuXZd/c8szkfSYht2xIZ28HBIW5vZwAAmnXwp5YD5AA5QA6QA2iv2V"
    "QsmjBNE4wxhDIE9zlcdwUp9x8rewEYRhGNZhPl8rvY2nYrMBoOsFj83AtE+RGUKxWcdz8lBgcATSug/fEUtRNbObgy"
    "ACEEnc4ZKE2XHx9/wJFVVQewLCtVdHp2"
)

PROMPT = r"""
yamlは絶対に更新すること
サーバーはポート3000で起動しているので絶対に新しく起動しないこと
テストはインスペクターを絶対に開かないこと
@nuxtjs/supabaseは使用禁止、純正クライアントを拡張してしようすること
fakerの使用禁止
キャッシュのクリアは絶対に行わないこと
絶対にデバッグモードでテストを実行せず、インスペクターを使用しないでテストを実行すること
必ず各テストでユーザー登録、ログインを行った後に該当ページに遷移し、テストデータをUIから作成し、該当ページに遷移しテストを行うこと
supabaseの接続情報は既存のnuxt.config.jsの値を使用し絶対に勝手に値が存在する想定で使用しないこと
supabaseはローカル環境も使用しないこと
.envファイルの内容は絶対に変更しないこと
設定値は以下であり絶対に変更しないこと
```
SUPABASE_UR
SUPABASE_ANON_KEY
...
BASE_URL
```
編集禁止ファイルは絶対に編集できないようにロックをかけています
...
プロンプト仕様:
  ...
  # ------------------------------------------------------------
    フィールド:
      必須リソース:
        - @20250320142446_initial_schema.sql   # [理由] DBスキーマからユニット抽出を行うため、設計の起点として必須。
        - @database.ts                        # [理由] DB型整合性を保証するため、Supabase型定義ファイルを必須とする。
...
# === 設定 ===
OFFSET_X = 39       # 画面右端からの↑アイコン中心までのXオフセット
# OFFSET_Y = 172      # 画面下端からの↑アイコン中心までのYオフセット
OFFSET_Y = 155      # 画面下端からの↑アイコン中心までのYオフセット
PASTE_OFFSET_Y = -35  # ↑アイコンの下にあるテキストボックスまでの相対Y位置
...
def load_reference_image():
    decoded = base64.b64decode(arrow_reference_base64)
    return Image.open(BytesIO(decoded))

def get_arrow_region():
    screen_width, screen_height = pyautogui.size()
    x1 = screen_width - OFFSET_X - REGION_WIDTH // 2
    y1 = screen_height - OFFSET_Y - REGION_HEIGHT // 2
    x2 = x1 + REGION_WIDTH
    y2 = y1 + REGION_HEIGHT
    return ImageGrab.grab(bbox=(x1, y1, x2, y2))

def is_prompt_ready(current_img, reference_img, threshold=37):
    diff = ImageChops.difference(reference_img.convert("RGB"), current_img.convert("RGB"))
    stat = ImageStat.Stat(diff)
    mean = sum(stat.mean) / len(stat.mean)
    print(f"[チェック] 類似度（平均差分）: {mean:.2f}")
    return mean < threshold

# === メイン処理 ===
def automate_cursor_prompt():
    reference_img = load_reference_image()
    print("== スクリプト起動 ==")
    print("準備↑アイコン検出を開始します（Ctrl+Cで停止）。")

    try:
        while True:
            region_img = get_arrow_region()
            print("[ループ] スクリーン領域を取得中...")
            if is_prompt_ready(region_img, reference_img):
                print("✅ マッチ → プロンプト貼り付け・送信処理を開始")

                screen_width, screen_height = pyautogui.size()
                paste_x = screen_width - OFFSET_X
                paste_y = screen_height - OFFSET_Y + PASTE_OFFSET_Y

                pyautogui.click(paste_x, paste_y)
                print(f"[クリック] 入力位置へ移動: ({paste_x}, {paste_y})")

                pyperclip.copy(PROMPT)
                pyautogui.hotkey('ctrl', 'v')
                time.sleep(0.3)

                pyautogui.click(screen_width - OFFSET_X, screen_height - OFFSET_Y)
                print("✅ ↑クリック → 次の待機へ\n")
